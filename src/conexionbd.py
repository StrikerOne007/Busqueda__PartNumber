...
class Conexion_BD:
    def __init__(self):
        try:
            ...
            print(
                f"Conexión exitosa a la base de datos en {time.time() - inicio} segundos"
            )
        except Error as e:
...
class DAO:
    ...
    def listar_data(self, consulta):
        if self.db.conexion.is_connected():
            try:
                ...
                print(
                    f"Se obtuvo el siguiente error la listar datos en base de datos: {ex}"
                )
                return None
    ...
    def carga_data_sql(self, consulta, columns):
        try:
            ...
            print(
                f"Se obtuvieron {resultado.shape[0]} datos en {time.time() - inicio} segundos"
            )
            return resultado
        except ValueError as e:
            print(
                f"El numero de columnas recibidos no coincide con los campos asignados {e} - carga_data_sql"
            )
